...
import { saveHighScore, getHighScore } from './components/highScoreUtils'

function App () {
  const initialGrid = [
    8, 0, 0, 0, 2, 0, 9, 1, 0, 2, 3, 4, 5, 1, 0, 0, 0, 7, 7, 1, 0, 0, 8, 0, 0,
    5, 4, 6, 0, 0, 1, 0, 0, 3, 0, 5, 1, 8, 5, 0, 0, 0, 7, 2, 0, 0, 4, 0, 6, 0,
    2, 8, 0, 0, 0, 6, 8, 0, 0, 0, 4, 0, 0, 0, 0, 0, 0, 0, 0, 1, 6, 2, 0, 0, 0,
    4, 0, 7, 5, 3, 0
  ]
  ...
  const initialSolvedGrid = [
    8, 5, 6, 7, 2, 4, 9, 1, 3, 2, 3, 4, 5, 1, 9, 6, 8, 7, 7, 1, 9, 3, 8, 6, 2,
    5, 4, 6, 9, 2, 1, 7, 8, 3, 4, 5, 1, 8, 5, 9, 4, 3, 7, 2, 6, 3, 4, 7, 6, 5,
    2, 8, 9, 1, 5, 6, 8, 2, 3, 1, 4, 7, 9, 4, 7, 3, 8, 9, 5, 1, 6, 2, 9, 2, 1,
    4, 6, 7, 5, 3, 8
  ]
  ...
  const highScoreEasy = getHighScore('Easy')
  const highScoreMedium = getHighScore('Medium')
  const highScoreHard = getHighScore('Hard')
  ...
  function _generateUniqueSudoku (solvedGrid, e) {
    ...
    const boxCounts = [0, 0, 0, 0, 0, 0, 0, 0, 0]
    let boxesAvailable = []
    let cellsAvailable = []
  ...
  function _createNewGame (e) {
    ...
    ;[...str].forEach((value, index) => {
      tempInitArray[index] = value === '.' ? 0 : value
    })
  ...
  function _isSolved (givenIndex, givenValue) {
    if (
      gameGrid.every((cell, cellIndex) => {
        if (cellIndex === givenIndex)
          return givenValue === solvedGrid[cellIndex]
        else return cell === solvedGrid[cellIndex]
      })
    ) {
      return true
    }
  ...
  function handleGameWon() {
    ...
    const elapsedTime = moment().diff(timeGameStarted, 'seconds');
    saveHighScore(elapsedTime, gameDifficulty); 
      ...
      <div className={overlay ? 'container blur' : 'container'}>
        ...
        <div className='innercontainer'>
          ...
          <div className='high-scores'>
            <HighScore gameMode='Easy' highScore={highScoreEasy} />
            <HighScore gameMode='Medium' highScore={highScoreMedium} />
            <HighScore gameMode='Hard' highScore={highScoreHard} />
          </div>
        </div>
        <Footer />
      </div>
